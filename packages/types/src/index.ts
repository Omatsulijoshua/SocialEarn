...
export type WatchSessionStatus = 'STARTED' | 'ACTIVE' | 'PAUSED' | 'COMPLETED' | 'VERIFICATION_PENDING' | 'VERIFIED' | 'REJECTED' | 'EXPIRED';
export type WithdrawalStatus = 'REQUESTED' | 'UNDER_REVIEW' | 'APPROVED' | 'PROCESSING' | 'COMPLETED' | 'REJECTED' | 'CANCELLED';
export type AuditAction = 
  | 'LOGIN'
  | 'USER_SUSPENDED'
  | 'USER_BANNED'
  | 'RESTRICTED_USER_ACCOUNT'
  | 'CAMPAIGN_APPROVED'
  | 'CAMPAIGN_REJECTED'
  | 'CAMPAIGN_PAUSED'
  | 'CAMPAIGN_REFUNDED'
  | 'APPROVED_EXTERNAL_CAMPAIGN'
  | 'REJECTED_EXTERNAL_CAMPAIGN'
  | 'EXTERNAL_LINK_APPROVED'
  | 'EXTERNAL_LINK_REJECTED'
  | 'REWARD_RULE_CHANGED'
  | 'POINT_CONVERSION_CHANGED'
  | 'APPROVED_CASH_WITHDRAWAL'
  | 'REJECTED_CASH_WITHDRAWAL'
  | 'WITHDRAWAL_APPROVED'
  | 'WITHDRAWAL_REJECTED'
  | 'PLATFORM_DISABLED'
  | 'ADMIN_CREATED';
